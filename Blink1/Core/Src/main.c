...
uint32_t blinkDelays[] =
		{ 1000, 500, 250, 100 };

uint16_t blinkDelay = 0;
uint8_t delaysIndex = 0;
...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
// Change &huart1 to &huart2 (or &huart3) depending on your board
...
int main(void)
{
	...
	MX_USART2_UART_Init();
	/* USER CODE BEGIN 2 */
	printf("Starting Blink1...\r\n");
	/* USER CODE END 2 */

	/* Infinite loop */
	/* USER CODE BEGIN WHILE */

	blinkDelay = blinkDelays[0];
	...
	while (1)
	{
		...
		if (buttonPressed)
		{
			...
			blinkDelay = blinkDelays[delaysIndex];

			buttonPressed = 0;

			printf("Blink Delay: %d. \r\n", blinkDelay);
		}
